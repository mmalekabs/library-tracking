...
30. [Arabic-insensitive search](#30-arabic-insensitive-search)
...
| `src/utils/arabicSearch.ts` | `normalizeArabicForSearch`, PostgreSQL Arabic-insensitive book/author/publisher search |
...
| `src/utils/arabicSearch.ts` | Client-side Arabic normalize for picker duplicate checks |
...
- `page`, `limit` (max 100)
- `search` — Arabic-insensitive (strips tashkeel; normalizes hamza/alef variants via `arabicSearch.ts`)
- `format`, `status`, `binding`
...
| `listBooks` | Paginated findMany; `search` uses `findBookIdsByArabicSearch` |
...
- Sidebar groups: Main, Library, Catalog, Import, Tools, Settings (collapsible; see `navGroups` in `AdminLayout.tsx`)
- **`reading-tracking` branch:** Reading + Add to read under Library group
...
- `navGroups` — collapsible sections with chevron toggle; group containing the current route auto-expands
  - **Main** — Dashboard
  - **Library** — Books, Reading (`end: true`), Add to read, To Purchase
...
**Add admin menu item:** add to the right `navGroups` entry + `pageTitles` + new route in `App.tsx`.
...
3. Add item to `navGroups` + `pageTitles` in `AdminLayout.tsx`
...
## 30. Arabic-insensitive search

Book, author, publisher, bookshelf, missing-info, and reading book-picker searches normalize Arabic text before matching.

### Normalization rules (`normalizeArabicForSearch`)

- Remove **tashkeel** and tatweel (ً ٌ ٍ َ ُ ِ ّ ْ ـ …)
- Map hamza/alef variants: **أ إ آ ٱ → ا**
- Map **ؤ → و**, **ئ / ى → ي**
- Lowercase Latin letters (ISBN / Goodreads Id search unchanged aside from case)

Example: query `رجلا` matches title `رجلاً`; `احمد` matches `أحمد`.

### Server implementation

| Function | File | Used by |
|----------|------|---------|
| `normalizeArabicForSearch` | `server/src/utils/arabicSearch.ts` | Pattern building |
| `findBookIdsByArabicSearch` | same | `bookService.listBooks`, missing info, `readingService.listReadableBooks` |
| `findAuthorIdsByArabicSearch` | same | `authorService`, `lookupService` |
| `findPublisherIdsByArabicSearch` | same | `publisherService`, `lookupService` |
| `findBookshelfIdsByArabicSearch` | same | `lookupService` |

PostgreSQL `regexp_replace` mirrors the JS rules so stored titles are matched without extra columns.

### Client

`client/src/utils/arabicSearch.ts` — `arabicSearchEquals` in `EntityPicker` / `TagPicker` when offering “Create …”.
