...
| [DETAILED.md](./DETAILED.md) | Deep reference when changing code | Every route, component, service, env var, migration step, Goodreads/Missing info, عصير الكتب price lookup, Bookmory import, Arabic search, grouped admin sidebar, merge entities, add-to-library flow, books table sort/columns, Add from Goodreads, Gift/Total value, entity tabs, and (on `reading-tracking` branch) the reading tracker — reading-only books, Goodreads add-to-read, session edit/delete, auto current page |
